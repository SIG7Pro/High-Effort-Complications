...
class Alphabet extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		...
		if (isMenuItem)
		{
			...
			//trace(fps);
			y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * 0.48), 0.16 * (60 / FlxG.updateFramerate));
			x = FlxMath.lerp(x, (targetY * 20) + 90, 0.16 * (60 / FlxG.updateFramerate));
		}
...
class AlphaCharacter extends FlxSprite
{
	...
	public function createBold(letter:String)
	{
		if(numbers.contains(letter) ||
		    symbols.contains(letter))
			return createLetter(letter, true);
		animation.addByPrefix(letter, letter.toUpperCase() + " bold", 24);
		animation.play(letter);
		updateHitbox();
	}

	public function createLetter(letter:String, rawData:Bool = false):Void
	{
